...
# (2) It suppresses warning message for locfdr. 

calmf1 = function(meanf, fy1, fy0, paraMRF) {
...
optimNR = function(paraIni = rep(0, 3), meanf, w1, alpha = 10^(-6), maxiter = 100) {
  w0 = 1 - w1
  iter = 1
  para = rep(1, 3)
  parapre = paraIni
...
getMRFDE_t_stats = function(zz, n_cell, n_gene, c_c, g_g,
                            nulltype = 1, df = 15, iterEM = 200, iterGibbsPost = 20000, brPost = 10000) {  
  
  z_scores = array(zz, dim = c(1, n_gene, n_cell))
  ...
  for (j in 1:iterEM) {
    ...
    tmp = optimNR(paraIni = rep(0, 3), meanf = mf1, w1 = w1, alpha = 10^(-6), maxiter = 100)
    if ((tmp=="NA")[1]){
      return("NA")
    }
